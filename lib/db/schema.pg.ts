...
// Postgres/Supabase variant of lib/db/schema.ts. Same 6 tables, same
// column semantics — only the dialect-specific builders differ (pgTable
// instead of sqliteTable, native timestamp/boolean/jsonb instead of the
// integer-mode/text-mode tricks SQLite needs).
//
// NOT YET VERIFIED against a live Postgres instance — the sandbox this was
// built in couldn't install Postgres locally (package mirror was down), so
// this was written carefully against standard Drizzle pg-core patterns but
// never actually run. Treat "run this against the real database and fix
// whatever breaks" as step one, not an afterthought — see
// AGENT_INSTRUCTIONS.md, Task 1.
