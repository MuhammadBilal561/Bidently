...
export async function GET(
  ...
  // Batch all drafts for these requirements in one query (no N+1).
  const reqIds = reqRows.map((r) => r.id);
  const draftRows = reqIds.length
    ? await db
        .select()
        .from(draftAnswers)
        .where(inArray(draftAnswers.requirementId, reqIds))
    : [];
  const draftsById = new Map(draftRows.map((d) => [d.requirementId, d]));
  ...
  const withDrafts: {
    ...
    return {
      ...
      status: r.status,
      keywords: r.keywords ?? [],
      draft: draft ? { answer: draft.content, content_gap: draft.contentGap } : null,
    };
  });
...
export async function DELETE(
  ...
  try {
    ...
    for (const r of reqRows) {
      await db.delete(draftAnswers).where(eq(draftAnswers.requirementId, r.id));
    }
