...
export async function GET(req: NextRequest) {
  ...
  const tenderRows = await db.select().from(tenders).where(eq(tenders.orgId, orgId)).all();
  const tenderIds = tenderRows.map((t) => t.id);
  const reqRows = tenderIds.length
    ? await db.select().from(requirements).where(inArray(requirements.tenderId, tenderIds)).all()
    : [];
  const draftRows = await db.select().from(draftAnswers).all();
  const draftCount = draftRows.filter((d) => reqRows.some((r) => r.id === d.requirementId)).length;
