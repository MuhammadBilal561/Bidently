...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const mfa = await db.select().from(mfaStates).where(eq(mfaStates.userId, session.userId)).get();
    ...
    await db.update(mfaStates).set({ enabled: false }).where(eq(mfaStates.userId, session.userId)).run();
