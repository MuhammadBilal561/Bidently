...
- The Postgres client (`lib/db/index.ts`) and the migration SQL under
  `lib/db/migrations-pg/` were verified by typecheck/lint/build but should be
  run against a live Supabase project (apply via `npm run db:migrate`) as the
  first deploy step — see `docs/OWNER_GUIDE.md` Phase E.

## Architecture rationale

- **Postgres / Supabase (production)** — the schema uses portable column types
  (JSONB for arrays, `real` for weights, text ids), so it works the same on
  local Postgres and serverless pools. Everything above the database layer only
  imports `db` and the schema objects.
...
The product positioning and "why Bidently" story lives in the project `README.md`.
